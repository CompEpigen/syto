...
def stage_dataframe(df, atlas, region_index, labels_dict, sample_id,
    ...
    prepared = atlas.prepare_reads(
        adapted, trim=True, labels_dict=labels_dict,
    ...
    counts = (
        adapted.groupby("original_label").size().reset_index(name="n_reads")
    )
...
def stage_file(csv_path, atlas, region_index, labels_dict, staged_dir, counts_dir,
