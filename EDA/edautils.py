...
def create_methylation_boxplots(df, min_methyl_labels_count =0):
    ...
    # Print sample sizes
    print("Sample sizes:")
    print(f"1. Correctly predicted (label == prediction, label != 39): {len(correctly_predicted)}")
    print(f"2. Incorrectly rejected (label != 39, prediction == 39): {len(incorrectly_rejected)}")
    print(f"3. Incorrectly predicted (label == 39, prediction != 39): {len(incorrectly_predicted)}")
    print(f"4. Correctly rejected (label == prediction == 39): {len(correctly_rejected)}")
    print()
    ...
    ax.set_title('Methylation Level Distribution by Prediction Outcome\n(label 39 = rejection class)', 
                 fontsize=14, fontweight='bold', pad=20)
    ax.grid(axis='y', alpha=0.3, linestyle='--')
    
    # Add sample size annotations
    for i, (data, group) in enumerate(zip(plot_data, groups), 1):
        ax.text(i, ax.get_ylim()[0], f'n={len(data)}', 
...
def create_methylation_violinplots(df, title= 'Methylation Level Distribution by Prediction Outcome\n(label 39 = rejection class)'):
    """
    ...
    correctly_predicted = df[(df['label'] == df['prediction']) & (df['label'] != 39)]
    incorrectly_rejected = df[(df['label'] != 39) & (df['prediction'] == 39)]
    incorrectly_predicted = df[(df['label'] == 39) & (df['prediction'] != 39) & (df['prediction'] != df['original_label'])]
    correctly_rejected = df[(df['label'] == 39) & (df['prediction'] == 39)]
    ...
    group_labels = ['Correctly\nPredicted', 'Incorrectly\nRejected', 
                    'Incorrectly\nPredicted', 'Correctly\nRejected']
    ax.set_xticks(range(1, 5))
    ax.set_xticklabels(group_labels, fontsize=11, fontweight='bold')
    
    # Add sample sizes below group labels
    sample_sizes = [len(correctly_predicted), len(incorrectly_rejected), 
                    len(incorrectly_predicted), len(correctly_rejected)]
    ...
    legend_elements = [
        Patch(facecolor=colors[0], alpha=0.7, edgecolor='black', label='Correctly Predicted'),
        Patch(facecolor=colors[1], alpha=0.7, edgecolor='black', label='Incorrectly Rejected'),
        Patch(facecolor=colors[2], alpha=0.7, edgecolor='black', label='Incorrectly Predicted'),
        Patch(facecolor=colors[3], alpha=0.7, edgecolor='black', label='Correctly Rejected')
    ]
    ax.legend(handles=legend_elements, loc='upper left', bbox_to_anchor=(1, 1), fontsize=10, framealpha=0.9)
...
def aggregate_predictions_by_dmr(
    ...
    if prediction_cols is None:
        prediction_cols = [col for col in df.columns if col.startswith('prediction_') and col != 'prediction']
    ...
    if weight_col not in df.columns and create_weight_from_cpgs:
        if 'methylated_CpGs' in df.columns and 'unmethylated_CpGs' in df.columns:
            ...
            raise ValueError(f"Weight column '{weight_col}' not found and cannot create from CpG columns.")
...
def get_final_prediction(
    ...
    for method in methods:
        ...
        pred_cols = sorted(pred_cols, key=lambda x: int(x.replace(prediction_prefix, '').replace(suffix, '')))
...
def uxm_deconvolution(atlas, ref_cells, sf, counts, sample_names=["pseudo_balk_sample"]):
    ...
    params = [(sf[['name', 'direction', samp]],
               ...
               False,
               False) for samp in sample_names]

    arr = [decon_single_samp(*p) for p in params]
    return arr 

def rearange_uxm_deconvolution_results(labels_dict_reversed, uxm_proportions, ref_cells):
    '''
    ...
    uxm_proportions_alligned = [uxm_proportions[i] for i in [int(np.where(ref_pos==i)[0][0]) for i in range(0,39)]]
    return uxm_proportions_alligned


# TODO: make sure that reference cellss are matching labels --> probably need to be reordered
def generate_pseudo_bulk(total_samples, labels, proportions, train_data, valid_data, test_data,atlas,ref_cells,labels_dict_reversed):
    ref_pos =np.array([labels_dict_reversed[cell] if cell in labels_dict_reversed.keys() else -1 for cell in ref_cells])
    ...
    for df in [train_data, valid_data, test_data]:
        ...
        sub = pd.concat(sub)
        sub_aggregated = aggregate_predictions_by_dmr(sub, group_cols=["dmr_ctype_label", "dmr_ctype"])
        sub_aggregated = sub_aggregated[target_columns]        
        subs.append(sub_aggregated)
        results = sub
        results_agg = results.groupby(["name","direction"]).aggregate({'record_M' : 'sum','record_U' : 'sum','record_X': 'sum'}).reset_index()
...
def init_worker(train_data, valid_data, test_data, allowed_labels, n_cells_max, n_read_per_split):
    ...
    _worker_data['grouped_train'] = train_data.groupby(["original_label", "dmr_ctype_label"], sort=False)

    _worker_data['grouped_valid'] = valid_data.groupby(["original_label", "dmr_ctype_label"], sort=False)

    _worker_data['grouped_test'] = test_data.groupby(["original_label", "dmr_ctype_label"], sort=False)
    ...
    for idx in batch_indices:
        # try:
            labels, proportions = random_select_with_weights(
                _worker_data['allowed_labels'], 
                _worker_data['n_cells_max']
            )
...
def generate_pseudo_bulk_optimized(total_samples, labels, proportions, 
    ...
    for grouped in [grouped_train, grouped_valid, grouped_test]:
        ...
        if sub_parts:
            ...
            results = sub
            results_agg = results.groupby(["name", "direction"]).aggregate({
                'record_M': 'sum',
                'record_U': 'sum',
                'record_X': 'sum'
            }).reset_index()
            ...
            sub = aggregate_predictions_by_dmr_optimized(sub, group_cols=["dmr_ctype_label", "dmr_ctype"])
...
def run_ios_generation_parallel(train_data, valid_data, test_data, file_name, n_io_examples=30000, 
    ...
    with ProcessPoolExecutor(
        ...
        # Submit all batches
        futures = {executor.submit(worker_task, batch): i for i, batch in enumerate(batches)}
        ...
        with tqdm(total=n_io_examples, desc="Generating examples") as pbar:
            ...
            for future in as_completed(futures):
                ...
                if len(all_ios) % checkpoint_interval < batch_size:
                    ...
                    all_ios = [] # Initialize from the beggining so the object is not growing in memory
...
class FlattenMLPDeconvolver(nn.Module):
    ...
    """
    def __init__(self, n_dmr_groups=39, n_classes=40, n_cell_types=39, hidden_dims=[512, 256, 128]):
        super().__init__()
...
class DMRAttentionDeconvolver(nn.Module):
    ...
    """
    def __init__(self, n_dmr_groups=39, n_pred_classes=40, n_cell_types=39, embed_dim=64,num_heads=4):
        super().__init__()
...
class DeconvolverOutput:
    ...
    combined_features: torch.Tensor    # (batch, combined_dim) - pre-head combined features
...
class DiagonalAwareDeconvolver(nn.Module):
    ...
    - Ablation controls to zero-out feature pathways
    ...
    def __init__(
        ...
        self.diag_feature_dim = hidden_dim
        self.confusion_feature_dim = hidden_dim
        self.reject_feature_dim = hidden_dim // 2
        self.combined_dim = hidden_dim + hidden_dim + hidden_dim // 2
        
        # Diagonal processing: how much does DMR_i predict cell type i?
        ...
        # Off-diagonal encoder: confusion patterns
        ...
        # Rejection pattern (column 40) — informative about noise/ambiguity
        self.reject_encoder = nn.Sequential(
            nn.Linear(n_dmr_groups, hidden_dim // 2),
            nn.GELU()
        )
    ...
    def extract_input_features(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        Returns:
            ...
            - diagonal: (batch, n_dmr) - diagonal elements
            - reject_col: (batch, n_dmr) - rejection column
            - off_diagonal: (batch, n_dmr, n_dmr) - off-diagonal matrix
            - full_matrix: (batch, n_dmr, n_pred_classes) - original matrix
        """
        batch_size = x.shape[0]
        
        # Extract diagonal: x[i, i] for i in 0..n_dmr-1
        diagonal = torch.diagonal(x[:, :, :self.n_dmr], dim1=1, dim2=2)  # (batch, 39)
        
        # Rejection column (last column)
        reject_col = x[:, :, -1]  # (batch, 39)
        
        # Off-diagonal elements (zero out diagonal for isolation)
        cell_type_matrix = x[:, :, :self.n_dmr].clone()  # (batch, 39, 39)
    ...
    def encode_features(
        ...
        """
        Encode input through the three pathways with optional ablation.
        
        Args:
            x: Input tensor of shape (batch, n_dmr_groups, n_pred_classes)
            zero_diagonal: If True, zero out diagonal features
            zero_confusion: If True, zero out confusion features
            zero_reject: If True, zero out rejection features
    ...
    def forward(
        ...
        Args:
            ...
            zero_diagonal: If True, zero out diagonal pathway
            zero_confusion: If True, zero out confusion pathway
            zero_reject: If True, zero out rejection pathway
        ...
        # Combine features
        combined = torch.cat([diag_features, confusion_features, reject_features], dim=-1)
    ...
    def ablation_study(
        ...
        """
        Run systematic ablation study on feature pathways.
        ...
        Returns:
            Dictionary with predictions under each ablation condition
        """
        conditions = {
            'full_model': (False, False, False),
            'no_diagonal': (True, False, False),
            'no_confusion': (False, True, False),
            'no_reject': (False, False, True),
            'only_diagonal': (False, True, True),
            'only_confusion': (True, False, True),
            'only_reject': (True, True, False),
            'no_features': (True, True, True)
        }
        
        results = {}
        with torch.no_grad():
            for name, (zd, zc, zr) in conditions.items():
                pred = self.forward(x, zero_diagonal=zd, zero_confusion=zc, zero_reject=zr)
                results[name] = pred
    ...
    def get_feature_importance(
        ...
        Returns:
            Dictionary with importance scores for each input feature
        """
        ...
        importance = {
            ...
            'diagonal_importance': x.grad[:, :, :self.n_dmr].diagonal(dim1=1, dim2=2).detach(),
            'reject_importance': x.grad[:, :, -1].detach()
        }
...
def compute_deconvolution_metrics(pred: torch.Tensor, target: torch.Tensor, eps: float = 1e-8) -> dict:
    ...
    with torch.no_grad():
        ...
        # KL Divergence
        kl = (target * (target.clamp(min=eps).log() - pred.clamp(min=eps).log())).sum(dim=-1).mean().item()
...
def train_matrix_deconvolver(
    ...
    def loss_fn(pred, target, eps=1e-8):
        ...
        kl = (target * (target.clamp(min=eps).log() - pred.clamp(min=eps).log())).sum(dim=-1).mean()
    ...
    for epoch in range(n_epochs):
        ...
        if early_stopping.best_score is None:
            is_best = True
        elif es_mode == 'min' and current_metric < early_stopping.best_score - early_stopping_min_delta:
            is_best = True
        elif es_mode == 'max' and current_metric > early_stopping.best_score + early_stopping_min_delta:
            is_best = True
...
def plot_predictions_by_celltype(
    ...
    for ct_idx in range(n_cell_types):
        ...
        if show_regression_lines and not np.isnan(ct_r2):
            ...
            ax.plot(x_line, y_line, color=colors[ct_idx], linestyle='--', alpha=0.7, linewidth=1.5)
        ...
        legend_handles.append(
            Line2D([0], [0], marker='o', color='w', markerfacecolor=colors[ct_idx], 
    ...
    if n_cell_types > 10:
        ax.legend(
            ...
            fontsize=8, ncol=1 + n_cell_types // 20
        )
    else:
        ax.legend(
            legend_handles, legend_labels,
...
def bland_altman_plot(
    ...
    if target_complexity is not None:
        if isinstance(target_complexity, int):
            ...
            valid_samples = (complexity_per_sample >= min_c) & (complexity_per_sample <= max_c)
        else:
    ...
    if color_by == 'celltype':
        ...
        color_map = {ct: cmap_obj(i / max(len(unique_ct) - 1, 1)) for i, ct in enumerate(unique_ct)}
        ...
        colors = [cmap_obj((cx - min_cx) / max(max_cx - min_cx, 1)) for cx in complexity_labels]
    elif color_by == 'mean':
        cmap_obj = plt.get_cmap(cmap)
    ...
    # --- Plot points ---
    scatter = ax.scatter(means, diffs, c=colors, alpha=alpha, s=point_size, 
    ...
    if show_regression:
        ...
        ax.plot(x_range, y_reg, color='green', linestyle=linestyle, linewidth=1.5,
    ...
    if title is None:
        ...
        if target_celltype is not None:
            if len(celltype_indices) == 1:
                ct_name = labels_dict.get(celltype_indices[0], f'Cell Type {celltype_indices[0]}')
        ...
        if target_complexity is not None:
            if isinstance(target_complexity, int):
                ...
                title_parts.append(f'Complexity: {target_complexity[0]}-{target_complexity[1]}')
            else:
                title_parts.append(f'Complexity: {target_complexity}')
        
        title = ' | '.join(title_parts)
    
    ax.set_title(f'{title}\n(n={len(diffs)} points from {n_filtered_samples} samples)', fontsize=12)
    ...
    if color_by == 'celltype' and n_filtered_celltypes <= 20:
        handles = [Line2D([0], [0], marker='o', color='w', markerfacecolor=color_map[ct],
                          markersize=8, label=labels_dict.get(ct, f'CT{ct}'))
                   for ct in unique_ct]
        ax.legend(handles=handles, loc='upper left', fontsize=8, 
                  title='Cell Type', bbox_to_anchor=(1.02, 1))
    elif color_by == 'complexity':
        sm = plt.cm.ScalarMappable(cmap=cmap_obj, 
                                    norm=plt.Normalize(vmin=min_cx, vmax=max_cx))
        sm.set_array([])
        cbar = plt.colorbar(sm, ax=ax, label='Mixture Complexity')
    elif color_by == 'mean' or (show_density and isinstance(colors, np.ndarray)):
        plt.colorbar(scatter, ax=ax, label='Mean Proportion' if color_by == 'mean' else 'Density')
    ...
    if return_stats:
        stats_dict = {
            ...
            'percent_within_loa': np.mean((diffs >= loa_lower) & (diffs <= loa_upper)) * 100
        }
    ...
    **kwargs
    ...
    for item, label in zip(items, item_labels):
        try:
            ...
            # Apply filtering
            if filter_mode != 'all':
                _, _, mask = prop_filter.filter_data(true_vals, pred_vals, mode=filter_mode)
    ...
    for idx, (item, label) in enumerate(zip(items, item_labels)):
        ...
        if stat is None or 'error' in stat:
            error_msg = stat.get('error', 'Insufficient data') if stat else 'Insufficient data'
        ...
        if show_ci:
            ...
            ci_mean = (stat['mean_difference'] - t_crit * se_mean,
                      stat['mean_difference'] + t_crit * se_mean)
            ci_upper = (stat['loa_upper'] - t_crit * se_loa,
                       stat['loa_upper'] + t_crit * se_loa)
            ci_lower = (stat['loa_lower'] - t_crit * se_loa,
    ...
    for label, stat in all_stats.items():
        if stat is not None and 'error' not in stat:
            return_stats[label] = {k: v for k, v in stat.items() 
...
def bland_altman_comparison(
    ...
    ax.errorbar(x, biases, 
                yerr=[np.array(biases) - np.array(ci_lower), 
                      np.array(ci_upper) - np.array(biases)],
                fmt='none', color='black', capsize=5)
    ax.axhline(0, color='red', linestyle='--', linewidth=1)
    ax.set_xticks(x)
    ax.set_xticklabels(group_names if compare_by == 'celltype_group' else 
                       [str(c) for c in unique_complexities], rotation=45, ha='right')
    ...
    ax = axes[1]
    loa_widths = [comparison_stats[n]['loa_upper'] - comparison_stats[n]['loa_lower'] for n in names]
    ...
    colors = ['green' if p >= 95 else 'orange' if p >= 90 else 'red' for p in pct_within]
    ...
    fig.suptitle(f'Bland-Altman Comparison by {compare_by.replace("_", " ").title()}', 
                 fontsize=14, y=1.02)
...
def print_bland_altman_stats(stats: dict, name: str = ''):
    ...
    print(f"\n📊 Sample Size:")
    print(f"   Points: {stats['n_points']}, Samples: {stats['n_samples']}, Cell Types: {stats['n_celltypes']}")
    
    print(f"\n📈 Agreement Metrics:")
    print(f"   Mean Difference (Bias): {stats['mean_difference']:.4f}")
    print(f"   SD of Differences:      {stats['std_difference']:.4f}")
    print(f"   95% CI of Mean:         [{stats['ci_mean'][0]:.4f}, {stats['ci_mean'][1]:.4f}]")
    
    print(f"\n📏 Limits of Agreement (95%):")
    print(f"   Upper LoA: {stats['loa_upper']:.4f}  (95% CI: [{stats['ci_loa_upper'][0]:.4f}, {stats['ci_loa_upper'][1]:.4f}])")
    print(f"   Lower LoA: {stats['loa_lower']:.4f}  (95% CI: [{stats['ci_loa_lower'][0]:.4f}, {stats['ci_loa_lower'][1]:.4f}])")
...
def analyze_read_dmr_overlap(read_start, read_end, chrom, dmr_trees, strict=False):
    ...
    if strict and overlapping:
        overlapping = {
            iv for iv in overlapping 
            if iv.begin <= read_start and iv.end >= read_end
        }
...
def chunk_read_data(seq, methylation_encoding, cpg_positions, meth_states, 
                    read_start, read_end, chrom, chunk_size, dmr_trees, strict):
    """
    ...
    for chunk_start_offset in range(0, read_length, chunk_size):
        ...
        overlapping_dmrs = _get_overlapping_dmrs(chunk_gen_start, chunk_gen_end, chrom, dmr_trees, strict)
...
def process_single_read(read, data_type, methyl_tr=122, ref_fasta=None, 
                       interesting_chromosomes=None, min_mapq=10, require_flags=3, 
                       exclude_flags=1796, min_cpgs=1):
    """
...
def merge_paired_reads(df, verbose=True):
    ...
    if verbose:
        ...
        print(f"  Multiplets (>2): {len(multiplets)} (will be treated as separate entries)")
    ...
    if verbose:
        print(f"Merged result: {len(result_df)} fragments (from {len(df)} read entries)")
...
def _merge_mate_pair(mate1, mate2):
    ...
    merged_encoding, merged_cpg_stats = _merge_methylation_encodings(
        mate1['seq'], mate1['methylation_encoding'], mate1['read_start'],
        mate2['seq'], mate2['methylation_encoding'], mate2['read_start'],
        frag_start, frag_end
    )
    ...
    if 'seq_clipped' in mate1.index and dmr_start and dmr_end:
        ...
        clipped_encoding, clipped_stats = _merge_methylation_encodings(
            mate1.get('seq_clipped', ''), mate1.get('methylation_clipped', ''), 
            mate1.get('clip_start', mate1['read_start']),
            mate2.get('seq_clipped', ''), mate2.get('methylation_clipped', ''), 
        ...
        clipped_methylated = mate1.get('clipped_methylated', 0) + mate2.get('clipped_methylated', 0)
        clipped_unmethylated = mate1.get('clipped_unmethylated', 0) + mate2.get('clipped_unmethylated', 0)
    ...
    merged = {
        ...
        # DMR info (singular)
        'overlaps_dmr': mate1.get('overlaps_dmr', False) or mate2.get('overlaps_dmr', False),
        'dmr_label': dmr_label,
        'dmr_type': dmr_type,
        'dmr_start': dmr_start,
        'dmr_end': dmr_end,
...
def _merge_methylation_encodings(seq1, enc1, start1, seq2, enc2, start2, frag_start, frag_end):
    """
...
def add_reference_cpg_counts(df, reference_path, verbose=True):
    ...
    df['unknown_cpgs'] = df['ref_cpg_count'] - df['methylated_cpgs'] - df['unmethylated_cpgs']
    ...
    if verbose:
        ...
        print(f"Reference CpG counts - Total: {total_ref}, M: {total_m}, U: {total_u}, X: {total_x}")
...
def process_tabular_chunk(args):
    ...
    """
    (chrom, chunk_start, chunk_end, bam_path, 
     interesting_chromosomes, estimate_cov, 
     methyl_tr, data_type, reference_path, min_mapq, require_flags, exclude_flags, min_cpgs) = args
...
def process_bam_with_chunking(bam_path, chromosomes, 
                               n_jobs=4, chunk_size_genomic=1_000_000, methyl_tr=122,
                               reference_path=None, data_type=None,
                               min_mapq=10, require_flags=3, exclude_flags=1796, min_cpgs=1,
                               merge_pairs=True):
    """
    ...
    for chrom in chromosomes:
        ...
        for start in range(0, chr_len, chunk_size_genomic):
            ...
            tasks.append((chrom, start, end, bam_path, chromosomes, 
                         False, methyl_tr,
                         data_type, reference_path, min_mapq, require_flags, exclude_flags, min_cpgs))
...
class DeconvolverVisualizer:
    ...
    def __init__(
        ...
        self.model = model
        self.cell_type_names = cell_type_names or [f'CT_{i}' for i in range(model.n_cell_types)]
    ...
    def plot_ablation_comparison(
        ...
        for idx, condition in enumerate(conditions):
            ...
            if y_true is not None:
                ...
                ax.scatter(range(len(true)), true, color='red', marker='x', 
                          s=50, zorder=5, label='True')
            ...
            ax.set_xticks(top3)
            ax.set_xticklabels([self.cell_type_names[i] for i in top3], 
    ...
    def plot_feature_space(
        ...
        features = torch.cat([
            getattr(o, feature_map[feature_type]) for o in outputs
        ...
        if labels is not None:
            ...
            scatter = ax1.scatter(embedded[:, 0], embedded[:, 1], 
                                 c=labels_np, cmap='tab20', alpha=0.7, s=50)
        ...
        ax2.axvline(feature_norms.mean(), color='red', linestyle='--', 
    ...
    def plot_diagonal_analysis(
        ...
        # Extract data
        diag_input = torch.diagonal(
            x[sample_idx, :, :self.model.n_dmr], dim1=0, dim2=1
        ).detach().cpu().numpy()
        ...
        bars = ax1.bar(range(len(diag_input)), diag_input, 
        ...
        for i in top5:
            ax1.annotate(self.cell_type_names[i], (i, diag_input[i]), 
        ...
        diag_feat_2d = diag_feat.reshape(-1, 16) if len(diag_feat) >= 16 else diag_feat.reshape(1, -1)
        sns.heatmap(diag_feat_2d, cmap='RdBu_r', center=0, ax=ax3, 
    ...
    def plot_confusion_patterns(
        ...
        ax1 = axes[0]
        sns.heatmap(matrix, cmap='YlOrRd', ax=ax1, 
        ...
        sns.heatmap(off_diag, cmap='YlOrRd', ax=ax2,
                   xticklabels=5, yticklabels=5,
        ...
        conf_feat_2d = conf_feat.reshape(-1, 16) if len(conf_feat) >= 16 else conf_feat.reshape(1, -1)
        sns.heatmap(conf_feat_2d, cmap='RdBu_r', center=0, ax=ax3,
                   cbar_kws={'label': 'Activation'})
    ...
    def plot_rejection_analysis(
        ...
        ax1.axhline(reject_col.mean(), color='blue', linestyle='--', 
        ...
        ax3 = axes[2]
        ax3.bar(range(len(reject_feat)), reject_feat, 
               color=plt.cm.RdBu_r((reject_feat - reject_feat.min()) / 
    ...
    def plot_pathway_contributions(
        ...
        with torch.no_grad():
            ...
            full_pred = self.model(x[sample_idx:sample_idx+1]).squeeze().cpu().numpy()
            
            # Only diagonal
            diag_only = self.model(
                x[sample_idx:sample_idx+1], 
                zero_confusion=True, zero_reject=True
            ).squeeze().cpu().numpy()
            
            # Only confusion
            conf_only = self.model(
                x[sample_idx:sample_idx+1],
                zero_diagonal=True, zero_reject=True
            ).squeeze().cpu().numpy()
            
            # Only rejection
            rej_only = self.model(
                x[sample_idx:sample_idx+1],
                zero_diagonal=True, zero_confusion=True
            ).squeeze().cpu().numpy()
        ...
        for ax, (name, pred), color in zip(axes.flat, predictions.items(), colors):
            ...
            for i in top3:
                if pred[i] > 0.01:
                    ax.annotate(f'{self.cell_type_names[i]}\n{pred[i]:.2f}',
                               (i, pred[i]), ha='center', va='bottom', fontsize=8)
        
        fig.suptitle(f'Pathway Contribution Analysis (Sample {sample_idx})', fontsize=14)
        plt.tight_layout()
        return fig
    ...
    def plot_feature_correlation_matrix(
        ...
        """
        # Stack all features
        diag = torch.cat([o.diag_features for o in outputs], dim=0).detach().cpu().numpy()
        conf = torch.cat([o.confusion_features for o in outputs], dim=0).detach().cpu().numpy()
        rej = torch.cat([o.reject_features for o in outputs], dim=0).detach().cpu().numpy()
        ...
        all_features = np.stack([
            diag_mean, conf_mean, rej_mean,
            diag_norm, conf_norm, rej_norm
        ], axis=1)
        
        feature_names = [
            'Diag Mean', 'Conf Mean', 'Rej Mean',
            'Diag Norm', 'Conf Norm', 'Rej Norm'
        ]
    ...
    def create_interpretability_report(
        ...
        with torch.no_grad():
            outputs = [
                self.model(x[i:i+1], return_features=True)
        ...
        all_outputs = [self.model(x[i:i+1], return_features=True) for i in range(len(x))]
        figures['feature_correlation'] = self.plot_feature_correlation_matrix(all_outputs)
...
def plot_methylation_with_predictions(df, figsize=(14, 6), colors=None):
    ...
    bp_data = [df[df['dmr_ctype_matched'] == tissue]['methylation_level'].values 
               for tissue in tissue_order]
    ...
    line = ax2.plot(x_positions, predictions, 
                    color='darkgreen', 
                    marker='D', 
                    markersize=10,
                    linewidth=2.5,
    ...
    for x, y, tissue in zip(x_positions, predictions, tissue_order):
        if not np.isnan(y):
            ax2.text(x, y + 0.03, f'{y:.3f}', 
                    ha='center', va='bottom', 
                    fontsize=9, fontweight='bold',
                    bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.8))
    
    ax2.set_ylabel('Average Prediction Score\n(for matching tissue type)', 
    ...
    from matplotlib.patches import Patch
    legend_elements = [
        Patch(facecolor='lightblue', edgecolor='black', label='Methylation Level (boxplot)'),
        plt.Line2D([0], [0], color='darkgreen', marker='D', markersize=10, 
                   linewidth=2.5, label='Avg Prediction Score')
    ]
    
    ax1.legend(handles=legend_elements, loc='upper left', fontsize=10)
    
    # Title
    plt.title('Methylation Levels vs Model Predictions by Tissue Type\n' + 
...
def plot_methylation_with_predictions_grouped(df, figsize=(16, 6)):
    ...
    for tissue in tissue_order:
        ...
        stats_data.append({
            ...
            'mean_prediction': tissue_data[pred_col].mean() if pred_col in tissue_data.columns else np.nan,
            'std_prediction': tissue_data[pred_col].std() if pred_col in tissue_data.columns else np.nan
        })
    ...
    bars = ax2.bar(x_pos, stats_df['mean_prediction'], 
    ...
    ax2.set_title('Model Predictions for Matching Labels', fontsize=13, fontweight='bold')
    ax2.set_ylim(-0.05, 1.05)
    ax2.grid(True, alpha=0.3, axis='y')
    ax2.set_xticks(x_pos)
    ax2.set_xticklabels([f'{tissue}\n(label={stats_df.iloc[i]["label"]})' 
...
def plot_methylation_vs_prediction_scatter(df, figsize=(12, 8)):
    ...
    for idx, tissue in enumerate(sorted(tissue_types)):
        ...
        if pred_col in tissue_data.columns:
            ...
            ax.plot(x_line, p(x_line), "r--", linewidth=2, alpha=0.8, label=f'Corr: {corr:.3f}')
    ...
    plt.suptitle('Methylation Level vs Model Prediction by Tissue Type', 
...
def plot_confusion_matrix_for_target_confidence(predictions, confidence, target_confidence, labels, labels_list, title="", exclude_rejected=True):
    confident_predictions = np.array([int(x) if y>target_confidence else 39 for (x,y) in zip(predictions, confidence)])
    ...
    for (i, j), val in np.ndenumerate(cf_matrix):
        ...
        if i == cf_matrix.shape[0]-1 or j == cf_matrix.shape[1]-1:
            color = "white" if val > (max_val_subset * 0.3) else "black" # Adjust contrast as needed
        else:
...
def prepare_reads_for_uxm(reads_data,atlas,labels_dict, cell_type_match_dict,
                          debug=False, unmethyl_tr = 0.25, methyl_tr = 0.75,seq_column = "seq", 
    ...
    for _, row in atlas.iterrows():
        ...
        while scan_ptr < n_records:
            ...
            if record["read_end"] >= start:
                ...
                if debug:
                    results.append((
                        chromosome, record[read_name_column], record["read_start"], record["read_end"],
    ...
    results["dmr_ctype_matched"] = results["dmr_ctype"].apply(lambda x: cell_type_match_dict[x] if x in cell_type_match_dict.keys()  else x)
    results["dmr_ctype_label"] = results["dmr_ctype_matched"].apply(lambda x: int(labels_dict_reversed[x]) if x in labels_dict_reversed.keys()  else None)
