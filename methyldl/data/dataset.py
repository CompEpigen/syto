...
class SupervisedDataset(Dataset):
    ...
    def __init__(self, 
        ...
        if data_interface == "csv":
            ...
            dna, methylation, labels = data[dna_col],data[meth_col], data[label_col]
...
def generate_example_data(sequence_length: int = 150,
    ...
    if include_cpg_methylation and not cpg_proportion:
        raise ValueError("Proportions for CpG methylation must be specified if it is included.")
    if include_m6a_methylation and not m6a_proportion:
        raise ValueError("Proportions for m6A methylation must be specified if it is included.")
    
    # Helper function to generate methylation sequence
    def generate_methylation_sequence(sequence: str, target: str, proportions: Dict[int, float]) -> str:
    ...
    for _ in range(num_samples):
        ...
        if include_cpg_methylation:
            cpg_methylation = generate_methylation_sequence(genome_sequence, "C", cpg_proportion)
            row.append(cpg_methylation)

        # Add m6a_methylation if required
        if include_m6a_methylation:
            m6a_methylation = generate_methylation_sequence(genome_sequence, "A", m6a_proportion)
            row.append(m6a_methylation)

        # Add labels if required
        if include_labels:
            row.append(random.choices((0,1), weights=(1,1), k=1)[0])  # Random 1 or 0 label
    ...
    # Slide window across the sequence
        for start_idx in range(0, read_length - target_read_length + 1, stride):
            ...
            if labeled_count >= min_labeled_cpgs:
                ...
                entropy_score = float('inf')  # Penalize sequences with too few labeled CpGs
...
def select_optimal_subsequence_rolling_window(
    ...
    if read_length <= target_read_length:
        return select_optimal_subsequence_vectorized(
            input_ids, methylation_ids, target_read_length,
            selection_criteria, min_labeled_cpgs, stride, entropy_weight_factor
        )
    ...
    for start_idx in range(0, read_length - target_read_length + 1, stride):
        ...
        if labeled_count >= min_labeled_cpgs:
            ...
            if meth_count > 0 and unmeth_count > 0:
                ...
                entropy_score = -(p_meth * np.log2(p_meth) + p_unmeth * np.log2(p_unmeth))
            else:
...
def extract_optimal_subsequence_dataframe_chunked(
    ...
    for chunk_start in tqdm(range(0, len(df), chunk_size), desc="Processing chunks"):
        ...
        for idx, row in chunk_df.iterrows():
            try:
                selected_dna, selected_meth, labeled_count, entropy_score = select_optimal_subsequence_rolling_window(
                    row['input_ids'], 
                    row['methylation_ids'],
                    target_read_length, selection_criteria, min_labeled_cpgs, stride, entropy_weight_factor
                )
...
def extract_optimal_subsequence_dataframe(
    ...
    for idx, row in tqdm(df.iterrows()):
        try:
            selected_dna, selected_meth, labeled_count, entropy_score = select_optimal_subsequence(
                row['input_ids'], 
                row['methylation_ids'],
                target_read_length,
                selection_criteria,
