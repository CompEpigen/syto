...
class DMRCrossAttentionClassifier(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.num_labels = config.num_labels
        ...
        # Cross-attention: DMR (query) attends to sequence (key/value)
    ...
    def forward(self, sequence_output, dmr_ids, attention_mask=None):
        """
        Cross-attention is sequence-length invariant because query size is fixed.
        Works for both 40-token and 510-token sequences.
        """
        batch_size = sequence_output.size(0)
        
        # Get DMR embeddings (fixed size: [batch, hidden])
        dmr_embeds = self.dmr_embedding(dmr_ids)
        
        # Reshape for cross-attention: [batch, 1, hidden]
        dmr_query = dmr_embeds.unsqueeze(1)
        
        # Cross-attention: DMR queries the sequence
        # query: [batch, 1, hidden]
        # key/value: [batch, seq_len, hidden] - seq_len can be anything!
        ...
        # attended_output: [batch, 1, hidden]
        # attention_weights: [batch, 1, seq_len]
        
        attended_output = attended_output.squeeze(1)  # [batch, hidden]
        
        # Combine with DMR context
        combined = torch.cat([attended_output, dmr_embeds], dim=-1)
        
        # Fusion and classification
        fused = self.context_fusion(combined)
        logits = self.classifier(fused)
        
        return logits, attention_weights.squeeze(1)  # [batch, seq_len]