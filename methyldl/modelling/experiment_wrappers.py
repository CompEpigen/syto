...
class AbstractMLFlowExperiment:
    ...
    def __init__(
        ...
        if experiment_name is None:
            raise ValueError(
                "Experiment name must be provided to initialize experiment"
            )
        ...
        print(
            f"Found {len(self.data_dirs)} data directories: {list(self.data_dirs.keys())}"
        )
    ...
    def _find_data_directories(self):
        ...
        if not data_dirs:
            raise ValueError(
                f"No directories with required parquet files found in {self.data_path}"
            )
        print(data_dirs)
        return data_dirs
    ...
    def _calculate_data_stats(self, data_path):
        ...
        try:
            ...
            stats = {
                ...
                "num_negative": (
                    int(len(df) - df["label"].sum()) if "label" in df.columns else 0
                ),
                "positive_ratio": (
                    float(df["label"].mean()) if "label" in df.columns else 0.0
                ),
                "avg_sequence_length": (
                    float(df["input_ids"].str.len().mean())
    ...
    def _calculate_metrics(
        ...
        try:
            ...
            metrics = {
                ...
                "nan_percentage": (
                    float(num_nans_proba / num_total * 100) if num_total > 0 else 0.0
                ),
            ...
            if len(y_true_clean) > 0:
                ...
                metrics.update(
                    {
                        "accuracy": float(
                            accuracy_score(y_true_clean, y_pred_binary_clean)
                        ),
                        "precision": float(
                            precision_score(
                                y_true_clean,
                        ...
                        "mcc": float(
                            matthews_corrcoef(y_true_clean, y_pred_binary_clean)
                        ),
                    }
                )
                ...
                if len(np.unique(y_true_clean)) > 1:
                    if is_multiclass:
                        ...
                        try:
                            ...
                            print(
                                f"Warning: Could not calculate ROC AUC for multi-class: {e}"
                            )
                            metrics["roc_auc"] = 0.0
                    else:
                        # Binary ROC AUC
                        metrics["roc_auc"] = float(
                            roc_auc_score(y_true_clean, y_pred_proba_clean)
                        )
                else:
                    metrics["roc_auc"] = 0.0
                ...
                if not is_multiclass and cm.shape == (2, 2):
                    ...
                    per_class_precision = precision_score(
                        y_true_clean, y_pred_binary_clean, average=None, zero_division=0
                    )
            ...
            if metrics["nan_percentage"] > 5.0:
                print(
                    f"Warning: {metrics['nan_percentage']:.2f}% of samples contain NaN values"
                )
    ...
    def _aggregate_predictions(self, data_chunked, predictions, num_classes=2):
        ...
        if num_classes > 2 or (predictions.ndim == 2 and predictions.shape[1] > 1):
            ...
            for class_idx in range(predictions.shape[1]):
                data_chunked[f"predictions_proba_class_{class_idx}"] = predictions[
                    :, class_idx
                ]
            ...
            data_chunked["predictions_weighted"] = (
                predictions * data_chunked["num_cpgs"]
            )
        ...
        if is_multiclass:
            ...
            for class_idx in range(predictions.shape[1]):
                agg_dict[f"predictions_weighted_class_{class_idx}"] = pd.NamedAgg(
                    column=f"predictions_weighted_class_{class_idx}", aggfunc="sum"
                )
            ...
            for class_idx in range(predictions.shape[1]):
                data_chunked_agg[f"predictions_proba_class_{class_idx}"] = (
                    predictions_proba[:, class_idx]
                )
            ...
            # Binary classification
            data_chunked_agg = data_chunked.groupby("read_name").agg(
                predictions_weighted=pd.NamedAgg(
                    column="predictions_weighted", aggfunc="sum"
                ),
                num_cpgs=pd.NamedAgg(column="num_cpgs", aggfunc="sum"),
                label=pd.NamedAgg(column="label", aggfunc="min"),
            )
        ...
        predictions_binary = (
            data_chunked_agg["predictions"].values
            if not is_multiclass
            else predictions_binary
        )
            ...
            try:
                if training_args is not None:
                    results = self.train_dataset(
                        dataset_name, training_args=training_args, **training_kwargs
                    )
                else:
...
class TransformersMLFLowExperiment(AbstractMLFlowExperiment):
    def __init__(
        ...
        super().__init__(
            data_path, experiment_name, tracking_uri, max_sequence_length, splits
        )
...
class DismirMLflowExperiment(AbstractMLFlowExperiment):
    ...
    def _make_predictions_and_calculate_metrics(
        self, model, split_name, dataset_name, max_sequence_length
    ):
        ...
        try:
            ...
            if split_name not in ["train", "valid", "test"]:
                ...
                dna, methylation, labels = (
                    data_chunked["input_ids"],
                    data_chunked["methylation_ids"],
                    data_chunked["label"],
                )
            else:
            ...
            predictions, predictions_binary = model.predict(
                dna, methylation_sequences=methylation
            )
            ...
            if num_classes > 2:
                ...
                if predictions.ndim == 2:
                    for i in range(predictions.shape[1]):
                        pred_dict[f"predicted_probability_class_{i}"] = predictions[
                            :, i
                        ]
    ...
    def train_dataset(
        ...
        with mlflow.start_run(run_name=f"dataset_{dataset_name}"):
            ...
            dismir_instance = Dismir(
                self.max_sequence_length, *file_paths, flavour=self.model_flavor
            )
            ...
            if os.path.exists(best_weights_path):
                dismir_instance.model.load_state_dict(
                    torch.load(best_weights_path, map_location=dismir_instance.device)
                )
            ...
            for split in self.splits:
                ...
                metrics, predictions_df = self._make_predictions_and_calculate_metrics(
                    ...
                    split,
                    dataset_name,
                    dismir_instance.max_sequence_length,
                )
...
class EpigenBERT2MLflowExperiment(TransformersMLFLowExperiment):
    ...
    def _progressive_predict(self, data_df, checkpoint_path, split_name):
        ...
        if (
            split_name in ["train", "valid"]
            and max_read_length <= self.max_sequence_length
        ):
            print(
                f"Using standard prediction for {split_name} (max_length <= {self.max_sequence_length})"
            )
            ...
            predictions = (
                result.predictions if hasattr(result, "predictions") else result[0]
            )
        ...
        for max_seq_len in tqdm(sequence_ranges, desc=f"Processing {split_name}"):
            ...
            print(
                f"Processing {subset_data.shape[0]} sequences with max_length={max_seq_len}"
            )
    ...
    def _make_predictions_and_calculate_metrics(
        ...
        try:
            ...
            # Calculate metrics
            metrics = self._calculate_metrics(
                labels, predictions, predictions_binary, num_classes=num_classes
            )
    ...
    def train_dataset(
        ...
        with mlflow.start_run(run_name=f"dataset_{dataset_name}"):
            ...
            while attempt <= max_retries:
                ...
                print(
                    f"Weights sum for -2 layer: {np.sum(list(model_instance.model.parameters())[-2].to('cpu').detach().numpy())}"
                )
            ...
            checkpoint_path = self._get_best_checkpoint(
                trainer=model_instance.trainer, output_dir=training_args.output_dir
            )
...
class MethylBertMLflowExperiment(TransformersMLFLowExperiment):
    ...
    def _prepare_methylbert_list(self, data_path, split, dmrs, split_to_chunks=False):
        ...
        for i, row in data.iterrows():
            dna = generate_kmer_str_with_overlap(
                row["input_ids"][: (self.max_sequence_length + 2)]
            )
    ...
    def _make_predictions_and_calculate_metrics(
        self, model, split_name, dataset_name, train_dataset, valid_dataset, dmrs
    ):
        """Make predictions and calculate metrics for a given split."""
        try:
            path = self.data_dirs[dataset_name] / f"{split_name}.parquet"
            ...
            # Determine number of classes
            num_classes = (
                model.model.num_labels if hasattr(model.model, "num_labels") else 2
            )
        ...
        with mlflow.start_run(run_name=f"dataset{dataset_name}"):
            try:
                ...
                if training_args is not None:
                    ...
                    model_instance.training_args.per_device_train_batch_size = (
                        batch_size
                    )
                ...
                # -----------------------------------------------------------------------
                checkpoint_path = self._get_best_checkpoint(
                    trainer=model_instance.trainer, output_dir=output_dir
                )
                print(f"Using checkpoint: {checkpoint_path}")
                mlflow.log_param("best_checkpoint", checkpoint_path)
                # Log model artifacts
                ...
                for split in self.splits:
                    ...
                    metrics, predictions_df = (
                        self._make_predictions_and_calculate_metrics(
                            model_instance,
                            split,
                            dataset_name,
                            train_dataset,
                ...
                predictions_dir = os.path.join(
                    output_dir, f"predictions_{dataset_name}"
                )
                os.makedirs(predictions_dir, exist_ok=True)

                for split, pred_df in all_predictions.items():
                    pred_file = os.path.join(
                        predictions_dir, f"{split}_predictions.pkl"
                    )
                    with open(pred_file, "wb") as f:
                        pickle.dump(pred_df, f)
